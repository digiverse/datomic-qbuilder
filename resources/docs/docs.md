...
The `home-routes` handler in the `datomic-qbuilder.routes.home` namespace
...
The `home-page` function will in turn call the `datomic-qbuilder.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `datomic-qbuilder.handler` namespace:
...
Request middleware functions are located under the `datomic-qbuilder.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `datomic-qbuilder.dev-middleware` namespace found in
